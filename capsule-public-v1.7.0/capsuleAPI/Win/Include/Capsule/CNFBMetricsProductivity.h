...
/**
 * \brief Create a NFB classifier for productivity metrics after individual NFB has been calibrated
 *
 * \param session session handle. Valid session must be passed (does not need to be active)
 * \param nfb Calibrated individual nfb data
...
CL_DLL clCNFBMetricProductivity clCNFBMetricsProductivity_CreateCalibrated(
    clCSession session, const clCIndividualNFBData* nfb, clCError* error) NOEXCEPT;
/**
 ...
 */
CL_DLL void clCNFBMetricsProductivity_ImportBaselines(clCNFBMetricProductivity nfbMetrics,
                                                      const clCNFBMetricsProductivityBaselines* baselines) NOEXCEPT;

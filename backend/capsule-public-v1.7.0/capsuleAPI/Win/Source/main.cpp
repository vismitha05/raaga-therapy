...
void onUpdateUserState([[maybe_unused]] clCNFB nfb, const clCNFBUserState* userState) {
    ...
    std::cout << "NFB update state: alpha = " << userState->feedbackData[2] << " , beta = " << userState->feedbackData[4] << " , theta = " << userState->feedbackData[1] << std::endl;
}
...
void onSessionStarted(clCSession session) {
    ...
    clCError error = clC_Error_OK;
    clCIndividualNFBData nfb;
    clCNFBCalibrator_GetIndividualNFB(calibrator, &nfb, &error);
    if (error != clC_Error_OK) {
        std::cerr << "Failed to get infb metrics" << std::endl;
    }
    error = clC_Error_OK;
    productivity = clCNFBMetricsProductivity_CreateCalibrated(session, &nfb, &error);
    if (error != clC_Error_OK) {
...
void ClientLoop() {
    ...
    while (client) {
        ...
        if (deviceConnectionTime && s_time == deviceConnectionTime + 2 * kMsSec && !session) {
            ...
            if (error != clC_Error_OK) {
                std::cerr << "Failed to create session: " << error << std::endl;
                clientStopRequested = true;
                continue;
            }
